    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Publish a festival competition, cultural activity, or workshop.</p>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 overflow-hidden shadow-sm rounded-2xl p-6 sm:p-8">
                ...
                <form method="POST" action="{{ route('coordinator.events.store') }}" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title')" placeholder="e.g. Battle of the Bands, Web Hackathon" required autofocus />
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" class="block mt-1 w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm text-sm" rows="5" placeholder="Provide full event rules, timeline, prizes, and requirements..." required>{{ old('description') }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="category" name="category" class="block mt-1 w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm text-sm" required>
                                ...
                                <option value="seminar" @selected(old('category') == 'seminar')>Seminar</option>
                        ...
                        <div>
                            <x-input-label for="capacity_limit" :value="__('Capacity Limit (Max Seats)')" />
                            <x-text-input id="capacity_limit" class="block mt-1 w-full" type="number" name="capacity_limit" :value="old('capacity_limit', 100)" min="1" required />
                    ...
                    <div>
                        ...
                        <x-text-input id="venue" class="block mt-1 w-full" type="text" name="venue" :value="old('venue')" placeholder="e.g. Main Auditorium, CS Lab 2, Sports Complex" required />
                    ...
                    <!-- Status -->
                    <div>
                        <x-input-label for="status" :value="__('Initial Publication Status')" />
                        <select id="status" name="status" class="block mt-1 w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm text-sm" required>
                            <option value="published" @selected(old('status', 'published') == 'published')>Published (Visible on catalog)</option>
                            <option value="draft" @selected(old('status') == 'draft')>Draft (Hidden)</option>
                        </select>
